...
| Excessive evaluator cost or runtime | 64 MB suite limit, JSON depth/node limits, `--maximum-mutants`, evaluator timeout and batch controls, bounded request/response size, and changed-tool-only audits. | A permitted evaluator can still consume resources or call paid services. Enforce process, network, and provider-side budgets outside Mendmark. |
