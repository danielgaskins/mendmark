...
`ExactMatch` metrics. Mendmark plants 19 faults in the refund-agent case and
reports which faults that combination catches.

With Rubric 0.2.0 and the example configuration, 7 of 19 mutations are killed.
...
tool, omitted/unexpected/wrong-type arguments, or the duplicated refund at the
default efficiency threshold. Those 12 survivors show exactly where another
assertion or stricter policy is needed.
